...
export default class ConnectDetails extends LightningElement {
    ...
    handleEvidenceClick(event) {
        ...
        if (this.savedRTWOption !== null && this.savedRTWOption !== newRtwValue) {
            ...
            const child = this.template.querySelector('c-g-connect-rtw-upload');
    ...
    @wire(getMultiplePicklistValues, {
        objectName: 'Account',
        fieldNames: ['Right_to_work_document__c', 'Citizenship_Immigration_status__c', 'Type_of_licence__c', 'Additional_licence_categories__c']
    })
    ...
    async callConfirmDrivingLicense() {
        let allFieldsValid = true;
        const inputFields = this.template.querySelectorAll('.licenseNumber,.licenseExpiryDate,.typeOfLicense, .licenseIssueDate,.pointOfLicense');
        inputFields.forEach(inputField => {
            let value = inputField.value;
