...
export default class GConnectDriverProfile extends LightningElement {
    ...
    @track openRequestEvidence = false;
    @track isRequestingEvidence = false;
    ...
    closeEvidenceConfirmModal(){
        this.openRequestEvidence = false;
    }
    ...
    handleDownload(event) {
        ...
        if (docType == 'GConnect DL') {
            ...
            if (requiredDocCount === 1 && this.hasFrontRTWImage) {
                validData = true;
            }
            else if (requiredDocCount === 2 && this.hasFrontRTWImage && this.hasBackRTWImage) {
    ...
    handleDLFieldChange(event) {
        ...
        if (field === 'Driving_Licence_Issue_Date__c') {
            ...
            if (selectedDate > today) {
                ...
                return;
            } else if  (field === 'Driving_Licence_Expiry_Date__c') {
                const selectedDate = new Date(value);
                selectedDate.setHours(0, 0, 0, 0);
                ...
                if (selectedDate < today) {
                    event.target.setCustomValidity(
                        'Licence Date cannot be in the past'
                    );
                } else {
                    event.target.setCustomValidity('');
        ...
        try {
            ...
            for (const file of this.uploadFiles) {
                ...
                if (this.selectedOption === 'RTW' && this.recordData.hasAccessCode) {
                    fileDocName = `${this.recordData.First_Name__c} ${this.recordData.Last_Name__c}_RTW Check`;
                } else {
    ...
    onChangeRTWDetails(event) {
        ...
        if (fieldName === 'limitedHours') {
            ...
            if (!fieldValue || isNaN(value) || value === 0) {
                event.target.value = '';
                this.recordData.Limited_To_X_Hours_Per_Week__c = null;
                return;
            }

            const wholeNumber = Math.min(168, Math.max(1, Math.round(value)));
            event.target.value = wholeNumber;
            this.recordData.Limited_To_X_Hours_Per_Week__c = wholeNumber;
        }
    ...
    async handleSaveRTW() {
        ...
        if (this.recordData.hasShareCode && this.recordData.Type_of_e_visa__c === 'Time-limited right to work') {
            ...
            if (this.recordData.Any_work_restrictions__c === 'Yes') {
                ...
                const jobTypes = this.recordData.Limited_To_Specific_Job_Types__c;
                const otherRestrictions = this.recordData.Other_Restrictions__c;

                const hasLimitedHours =
                    hours !== null && hours !== undefined && hours.toString().trim() !== '';

                const hasJobTypes =
                    jobTypes && jobTypes.trim() !== '';
                ...
                if (!hasLimitedHours && !hasJobTypes && !hasOtherRestrictions) {
                    this.showToast(
                        'Error',
                        'Provide at least one work restriction: Hours per week, Specific Job Types, or Other Restrictions.',
                        'error'
                    );
                    return;
                }
                ...
                if (hasLimitedHours) {
                    ...
                    if (hoursNumber < 1 || hoursNumber > 168) {
                        this.showToast('Error', 'Hours must be between 1 and 168.', 'error');
                        return;
                    }
                }
            }
    ...
    openRequestEvidenceOpen(){
        this.openRequestEvidence = true;
    }
    ...
    async commitUploadedFiles() {
        ...
        for (const file of filesToUpload) {
            ...
            if (this.selectedOption === 'RTW') {
                ...
                if (this.recordData.hasAccessCode || this.recordData.hasShareCode) {
                    docType = 'Right To Work Check';
                    fileTitle = `${this.recordData.First_Name__c} ${this.recordData.Last_Name__c}_RTW Check`;
                } else {
                    docType = file.docType === 'Front'
                            ? 'Right To Work Front'
                            : 'Right To Work Back';
